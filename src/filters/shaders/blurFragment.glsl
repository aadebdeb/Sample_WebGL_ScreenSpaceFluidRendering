...
void main(void) {
    ...
    float centerLd = toLinearDepth(centerD.r, u_near, u_far);
    ...
    for (int i = 1; i < 5; i++) {
        ivec2 offset = u_horizontal ? ivec2(i, 0) : ivec2(0, i);
        {
            ...
            float ld = toLinearDepth(d.r, u_near, u_far);
            sum += weights[i] * mix(d, centerD, smoothstep(0.0, 0.1, abs(centerLd - ld)));
        }
        {
            vec3 d = texelFetch(u_srcTexture, clampCoord(coord - offset, texSize), 0).rgb;
            float ld = toLinearDepth(d.r, u_near, u_far);
